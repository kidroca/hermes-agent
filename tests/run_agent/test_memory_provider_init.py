...
def test_aiagent_forwards_notice_callbacks_to_memory_provider():
    """Background memory writes receive the agent's durable notice rail."""
    provider = RecordingMemoryProvider()
    cfg = {"memory": {"provider": "recording"}, "agent": {}}
    ...
    assert provider.init_kwargs["notice_callback"] == agent._emit_notice
    assert provider.init_kwargs["notice_clear_callback"] == agent._emit_notice_clear
